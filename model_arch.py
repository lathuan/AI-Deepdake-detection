# model_arch.py (ĐÃ SỬA LỖI TÌM LỚP NỀN DỰA TRÊN TYPE)
...
# Import cho việc kiểm tra loại lớp
from tensorflow.keras.src.engine.functional import Functional as FunctionalModel 

# --- HÀM TẠO MÔ HÌNH TWO-STREAM (Giữ nguyên, nhưng đảm bảo tên) ---
...
# --- HÀM TINH CHỈNH (FINE-TUNING) - ĐÃ SỬA LỖI TÌM LỚP BẰNG TYPE ---
def fine_tune_two_stream_model(model, learning_rate_finetune):
    
    xception_base = None
    resnet_base = None

    # Tìm lớp nền bằng kiểu dữ liệu (instanceof Functional/Model) và TÊN
    for layer in model.layers:
        # Kiểm tra xem lớp đó có phải là một Functional Model đã được gộp vào không
        if isinstance(layer, FunctionalModel) or isinstance(layer, Model):
    ...
    if xception_base is None or resnet_base is None:
        print("LỖI: KHÔNG THỂ TINH CHỈNH. Tên lớp nền Xception/ResNet50 vẫn không được tìm thấy.")
        # Nếu thất bại, chỉ biên dịch lại để chạy tiếp với LR mới (dù lớp nền vẫn đóng băng)
        model.compile(
