...
def upload_video():
    ...
    try:
        ...
        # Xử lý frames (SỬA: Thêm explanations với English fallback)
        frames_web = []
        for frame in result.get("frames_for_web", []):
            ...
            # English fallback nếu không có suspicious features: Luôn thêm ít nhất 1 neutral để hiển thị
            if not features:
                features = [{
                    'name': 'No suspicious features detected',
                    'description': 'All features appear natural, consistent with a real video.',
                    'severity': 'low',
                    'confidence': 0.0
                }]
                summary = "✅ The video appears REAL. No clear anomalies detected."
